...
function filteredChoice(data, key, choices) {
  ...
  if(value.length === 0){return data}else{return value}
}
...
const Feed = (fromHome) => {
  ...
  const map_query = () => {
    if (
      search_location[0] - 0.5 <= map.getBounds().getNorthEast().lat() &&
      search_location[0] + 0.5 >= map.getBounds().getSouthWest().lat() &&
      search_location[1] - 0.5 <= map.getBounds().getSouthWest().lng() &&
      search_location[1] + 0.5 >= map.getBounds().getNorthEast().lng()
    ) {
      setCenter(map.getCenter())
      fetch(`${process.env.BACKEND_URL}/map-query/?n=${map.getBounds().getNorthEast().lat()}&s=${map.getBounds().getSouthWest().lat()}&e=${map.getBounds().getNorthEast().lng()}&w=${map.getBounds().getSouthWest().lng()}`, {
        headers: {
              ...
              <Autocomplete
                ...
                onPlaceChanged={(e) => {
                  if (autocomplete !== null && autocomplete.getPlace().geometry !== undefined) {
                    ...
                    setSearch_location([autocomplete.getPlace().geometry.location.lat(), autocomplete.getPlace().geometry.location.lng()])
                  }
                }
                }
              >
...

        {
          (info_overlay) ?
            <h3 id="details" style={{display: { xs: "none", sm: "block" },}}>
              ...
              <Button href={`/${info_overlay.name_of_object_type}/${info_overlay.id}`}>Details</Button>
            </h3>
            : null
        }
